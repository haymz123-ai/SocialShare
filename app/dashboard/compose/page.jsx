...
function PlacementPicker({ profile, groupId, value, onChange, onLoaded }) {
  ...
  if (!placements.length) {
    if (profile.platform === 'facebook') return <div style={{ padding: '10px 16px', fontSize: 12, color: '#a16207', fontWeight: 600 }}>⚠ No Pages found. Reconnect and grant Page access.</div>
    if (profile.platform === 'pinterest') return <div style={{ padding: '10px 16px', fontSize: 12, color: '#dc2626', fontWeight: 600 }}>⚠ No boards found.</div>
    return null
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      {activeProfile && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ height: 2, flex: 1, background: `linear-gradient(90deg, ${info.color}50, transparent)`, borderRadius: 2 }} />
            <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', color: info.color }}>
              {PLATFORM_FORMATS[platform]?.find(f => f.value === format)?.label?.toUpperCase() || 'POST'} · {info.label.toUpperCase()}
            </span>
            <div style={{ height: 2, flex: 1, background: `linear-gradient(270deg, ${info.color}50, transparent)`, borderRadius: 2 }} />
          </div>
            ...
            {(['instagram', 'facebook', 'threads'].includes(platform)) && (
              <div style={{ background: '#16162a', borderRadius: isStory ? 18 : 13, border: `1px solid ${info.color}25`, overflow: 'hidden', boxShadow: `0 4px 24px ${info.color}18` }}>
                <div style={{ padding: '10px 13px', display: 'flex', alignItems: 'center', gap: 9, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  <div style={{ width: 32, height: 32, borderRadius: '50%', background: `linear-gradient(135deg,${info.color},${info.color}88)`, flexShrink: 0, border: `2px solid ${info.color}50` }} />
                  ...
                  <span style={{ fontSize: 16, color: 'rgba(255,255,255,0.25)', letterSpacing: 1 }}>···</span>
                </div>
                {hasMedia ? (
                  <div style={{ position: 'relative', width: '100%', background: '#0a0a18', overflow: 'hidden' }}>
                    {isVideo ? (
                      <video src={previewUrl} controls playsInline preload="metadata" style={{ width: '100%', maxHeight: isStory ? 460 : 300, objectFit: 'cover', display: 'block', background: '#000' }} />
                    ) : (
                    ...
                    {isReel && !isVideo && (
                      <div style={{ position: 'absolute', bottom: 10, right: 10, display: 'flex', flexDirection: 'column', gap: 14, alignItems: 'center' }}>
                        {['♥', '💬', '↗', '⋯'].map((ic, i) => (
                          <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                            <span style={{ fontSize: 20, color: '#fff', textShadow: '0 2px 8px rgba(0,0,0,0.8)' }}>{ic}</span>
                    ...
                    {mediaCount > 1 && !isReel && !isVideo && (
                      <div style={{ position: 'absolute', top: 9, right: 9, background: 'rgba(0,0,0,0.6)', borderRadius: 10, padding: '2px 8px', fontSize: 10, color: '#fff', fontWeight: 700 }}>1/{mediaCount}</div>
                    )}
                  </div>
                ) : (
                  !isStory && (
                    <div style={{ padding: '13px 13px 0', fontSize: 13, color: '#E8E4DF', lineHeight: 1.65, whiteSpace: 'pre-wrap', wordBreak: 'break-word', minHeight: 56 }}>
                ...
                {!isStory && (
                  <div style={{ padding: '9px 13px 13px' }}>
                    {hasMedia && (
                      <div style={{ display: 'flex', gap: 13, marginBottom: 7, alignItems: 'center' }}>
                        {['♥', '💬', '↗'].map((ic, i) => <span key={i} style={{ fontSize: 19, color: 'rgba(255,255,255,0.7)' }}>{ic}</span>)}
                        <span style={{ marginLeft: 'auto', fontSize: 19, color: 'rgba(255,255,255,0.7)' }}>🔖</span>
                      </div>
                    )}
                    {hasMedia && <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', marginBottom: 4 }}><span style={{ color: '#fff', fontWeight: 700 }}>1,024 likes</span></div>}
                    {postBody && (
                      <p style={{ fontSize: 12, color: '#E8E4DF', margin: 0, lineHeight: 1.55, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>
                        <strong style={{ marginRight: 4, color: '#fff' }}>your_account</strong>{postBody}
                      </p>
                    )}
                    {params.first_comment && (
                      <div style={{ marginTop: 6, padding: '5px 9px', borderRadius: 7, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            {platform === 'twitter' && (
              <div style={{ background: '#15202b', borderRadius: 13, border: '1px solid rgba(255,255,255,0.09)', padding: '15px', boxShadow: '0 4px 24px rgba(29,161,242,0.1)' }}>
                <div style={{ display: 'flex', gap: 10 }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg,#1DA1F2,#0d8ed9)', flexShrink: 0 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 13, color: '#E8E4DF', margin: '0 0 11px', lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-word', minHeight: 18 }}>
                      {postBody || <span style={{ opacity: .3, fontStyle: 'italic' }}>Your tweet here…</span>}
                    </p>
                    {hasMedia && (
                      <div style={{ borderRadius: 11, overflow: 'hidden', marginBottom: 11, border: '1px solid rgba(255,255,255,0.1)' }}>
                        {isVideo ? (
                          <video src={previewUrl} controls playsInline preload="metadata" style={{ width: '100%', maxHeight: 180, objectFit: 'cover', display: 'block', background: '#000' }} />
                        ) : (
                          <img src={previewUrl} alt="" style={{ width: '100%', maxHeight: 180, objectFit: 'cover', display: 'block' }} />
                        )}
                      </div>
                    )}
                    <div style={{ display: 'flex', gap: 18, color: 'rgba(255,255,255,0.35)' }}>
                      {['💬 12', '🔁 34', '♥ 284', '📊', '📤'].map((ic, i) => <span key={i} style={{ fontSize: 12 }}>{ic}</span>)}
                    </div>
                    {charCount > 240 && (
                      <div style={{ marginTop: 7, fontSize: 11, color: '#f59e0b', display: 'flex', alignItems: 'center', gap: 5, fontWeight: 600 }}>
            ...
            {platform === 'linkedin' && (
              <div style={{ background: '#1d2226', borderRadius: 13, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden', boxShadow: '0 4px 24px rgba(10,102,194,0.12)' }}>
                <div style={{ padding: '13px 15px', display: 'flex', gap: 9, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ width: 44, height: 44, borderRadius: 9, background: 'linear-gradient(135deg,#0A66C2,#0051a1)', flexShrink: 0 }} />
                  <div>
                  ...
                  <span style={{ marginLeft: 'auto', fontSize: 17, color: 'rgba(255,255,255,0.25)' }}>···</span>
                </div>
                <div style={{ padding: '11px 15px', fontSize: 13, color: '#E8E4DF', lineHeight: 1.65, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                  {postBody || <span style={{ opacity: .3, fontStyle: 'italic' }}>Your post content…</span>}
                </div>
                {hasMedia && (
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                    {isVideo ? (
                      <video src={previewUrl} controls playsInline preload="metadata" style={{ width: '100%', maxHeight: 200, objectFit: 'cover', display: 'block', background: '#000' }} />
                    ) : (
                      <img src={previewUrl} alt="" style={{ width: '100%', maxHeight: 200, objectFit: 'cover', display: 'block' }} />
                    )}
                  </div>
                )}
                <div style={{ padding: '9px 15px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 14, alignItems: 'center' }}>
                  {['👍', '💬', '🔁', '📤'].map((ic, i) => (
                    <span key={i} style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', display: 'flex', alignItems: 'center', gap: 4 }}>
                      <span style={{ fontSize: 13 }}>{ic}</span>{['Like', 'Comment', 'Repost', 'Send'][i]}
                    </span>
                  ))}
                </div>
              </div>
            ...
            {platform === 'tiktok' && (
              <div style={{ background: '#000', borderRadius: 18, border: `1px solid ${info.color}22`, overflow: 'hidden', boxShadow: `0 4px 24px ${info.color}15` }}>
                <div style={{ position: 'relative', background: '#111' }}>
                  {hasMedia && isVideo ? (
                    <video src={previewUrl} controls playsInline preload="metadata" style={{ width: '100%', maxHeight: 440, objectFit: 'cover', display: 'block', background: '#000' }} />
                    ...
                    <div style={{ paddingTop: '160%', position: 'relative', background: 'linear-gradient(to bottom, #1a0010, #000)' }}>
                      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <div style={{ width: 56, height: 56, borderRadius: '50%', border: `2px solid ${info.color}60`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <span style={{ fontSize: 18, marginLeft: 4 }}>▶</span>
                        </div>
                      </div>
                    </div>
                  )}
                  <div style={{ padding: '11px', background: 'linear-gradient(transparent,rgba(0,0,0,0.85))', position: hasMedia && isVideo ? 'relative' : 'absolute', bottom: 0, left: 0, right: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 9 }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: '#fff', marginBottom: 3 }}>@yourusername</div>
                        ...
                        <div style={{ marginTop: 4, fontSize: 10, color: 'rgba(255,255,255,0.45)', display: 'flex', alignItems: 'center', gap: 4 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, alignItems: 'center', flexShrink: 0 }}>
                        {[['♥', '284K'], ['💬', '1.2K'], ['🔁', '8.4K'], ['⋯', '']].map(([ic, ct], i) => (
                          <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                            <span style={{ fontSize: 22, textShadow: '0 2px 8px rgba(0,0,0,0.8)', color: i === 0 ? '#ff4d6d' : '#fff' }}>{ic}</span>
            ...
            {platform === 'youtube' && (
              <div style={{ background: '#0f0f0f', borderRadius: 13, border: '1px solid rgba(255,255,255,0.07)', overflow: 'hidden', boxShadow: '0 4px 24px rgba(255,0,0,0.1)' }}>
                <div style={{ position: 'relative', background: '#1a1a1a' }}>
                  {hasMedia && isVideo ? (
                    <video src={previewUrl} controls playsInline preload="metadata" style={{ width: '100%', maxHeight: 190, objectFit: 'cover', display: 'block', background: '#000' }} />
                  ) : hasMedia && !isVideo && previewUrl ? (
                    ...
                    <div style={{ position: 'relative', paddingTop: '56.25%', background: 'linear-gradient(135deg,#1a0000,#0f0f0f)' }}>
                      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <div style={{ width: 52, height: 52, borderRadius: 13, background: '#FF0000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <span style={{ fontSize: 20, marginLeft: 4 }}>▶</span>
                        </div>
                      </div>
                    </div>
                  )}
                  {!isVideo && <div style={{ position: 'absolute', bottom: 6, right: 8, background: 'rgba(0,0,0,0.85)', borderRadius: 5, padding: '2px 7px', fontSize: 10, color: '#fff', fontWeight: 700 }}>12:34</div>}
                </div>
                <div style={{ padding: '11px 13px 13px' }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#fff', lineHeight: 1.4, marginBottom: 5 }}>
                    {params.title || postBody?.slice(0, 60) || 'Your video title'}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 7 }}>
                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#FF0000', flexShrink: 0 }} />
                    <div>
                    ...
                    <button style={{ marginLeft: 'auto', padding: '5px 12px', borderRadius: 18, background: '#fff', border: 'none', fontSize: 11, fontWeight: 700, color: '#0f0f0f', cursor: 'pointer' }}>Subscribe</button>
                  </div>
                  {postBody && (
                    <div style={{ padding: '7px 9px', borderRadius: 7, background: 'rgba(255,255,255,0.05)', fontSize: 11, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                      {postBody}
                    </div>
                  )}
                </div>
            ...
            {platform === 'pinterest' && (
              <div style={{ background: '#111', borderRadius: 15, border: `1px solid ${info.color}22`, overflow: 'hidden', boxShadow: `0 4px 20px ${info.color}12` }}>
                <div style={{ position: 'relative' }}>
                  {hasMedia && previewUrl ? (
                    isVideo ? (
                      <video src={previewUrl} controls playsInline preload="metadata" style={{ width: '100%', maxHeight: 240, objectFit: 'cover', display: 'block', background: '#000' }} />
                    ) : (
                      <img src={previewUrl} alt="" style={{ width: '100%', maxHeight: 240, objectFit: 'cover', display: 'block' }} />
                    )
                  ) : (
                    <div style={{ height: 180, background: `linear-gradient(135deg, ${info.color}1a, rgba(0,0,0,0))`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <span style={{ fontSize: 44, opacity: .15 }}>📌</span>
                    </div>
                  )}
                  <div style={{ position: 'absolute', top: 9, right: 9, background: '#E60023', borderRadius: 18, padding: '6px 14px', fontSize: 12, fontWeight: 700, color: '#fff', boxShadow: '0 4px 12px rgba(230,0,35,0.4)' }}>Save</div>
                </div>
                <div style={{ padding: '11px 13px 13px' }}>
                  {params.title && <div style={{ fontSize: 14, fontWeight: 700, color: '#fff', marginBottom: 5 }}>{params.title}</div>}
                  {postBody && <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', margin: 0, lineHeight: 1.55 }}>{postBody}</p>}
                  {params.destination_link && (
                    <div style={{ marginTop: 7, fontSize: 11, color: '#E60023', display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(230,0,35,0.08)', padding: '4px 9px', borderRadius: 7 }}>
                      🔗 {params.destination_link}
                    </div>
                  )}
                </div>
...
function MediaUploader({ files, urls, onFilesChange, onUrlsChange }) {
  ...
  return (
    <div>
              ...
              {files.map((file, i) => {
                ...
                return (
                  <div key={i} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1.5px solid #FDE68A' }}>
                    ...
                    <button type="button" onClick={() => removeFile(i)} style={{ position: 'absolute', top: 3, right: 3, width: 18, height: 18, borderRadius: '50%', background: 'rgba(0,0,0,0.65)', border: 'none', color: '#fff', fontSize: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '3px 5px', background: 'rgba(0,0,0,0.6)', fontSize: 9, color: 'rgba(255,255,255,0.7)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{file.name}</div>
                  </div>
                )
              })}
            </div>
      ...
      {totalItems > 0 && (
        <div style={{ marginTop: 10, fontSize: 12, color: '#15803d', display: 'flex', alignItems: 'center', gap: 5, fontWeight: 700 }}>
...
export default function ComposePage() {
  ...
  useEffect(() => {
    ...
    setProfiles([]); setSelectedProfileIds([]); setPlacements({}); setLoadedPlacements({})
    setPlatformParams({}); setExpandedParams({}); setPostMsg({ type: '', text: '' })
    setLoadingProfiles(true)
    fetchProfiles(selectedGroup.id).then(data => setProfiles(data)).finally(() => setLoadingProfiles(false))
  }, [selectedGroup?.id])
...
function buildPlatformsPayload() {
  ...
  const refreshProfilesBtn = (
    ...
    >
      <span style={{ display: 'inline-block', fontSize: 12, animation: refreshingProfiles ? 'spin 0.7s linear infinite' : 'none', transformOrigin: 'center' }}>↻</span>
        ...
        <form onSubmit={handlePost} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <div key={profile.id} style={{
                      ...
                      <div className="ptoggle" onClick={() => !inactive && toggleProfile(profile.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', cursor: inactive ? 'not-allowed' : 'pointer' }}>
                        ...
                        {isSelected && (() => {
                          ...
                          return fmtLabel ? (
                            <span style={{ fontSize: 10, color: info.color, background: info.color + '12', border: `1px solid ${info.color}30`, padding: '2px 8px', borderRadius: 6, fontWeight: 700, flexShrink: 0 }}>{fmtLabel.icon} {fmtLabel.label}</span>
                          ) : null
                        })()}
                        {inactive && <span style={{ fontSize: 11, color: '#dc2626', background: '#fee2e2', border: '1px solid #fca5a5', padding: '3px 8px', borderRadius: 6, fontWeight: 700 }}>Reconnect</span>}
                      </div>

                      {needsPlacement && (
                        <div style={{ borderTop: `1.5px solid ${info.color}18`, background: `${info.color}04` }}>
          ...
          {postMsg.text && (
            <div style={{
              ...
              background: postMsg.type === 'error' ? '#fee2e2' : '#dcfce7',
              border: `1.5px solid ${postMsg.type === 'error' ? '#fca5a5' : '#bbf7d0'}`,
              color: postMsg.type === 'error' ? '#dc2626' : '#15803d',
