...
function StatCard({ label, value, icon }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 22, fontWeight: 900, color: '#1a1040', letterSpacing: '-0.03em', lineHeight: 1 }}>
        {typeof value === 'number' ? value.toLocaleString() : '—'}
      </div>
      <div style={{ fontSize: 10, fontWeight: 700, color: '#9c8fc0', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{label}</div>
    </div>
  )
}
...
function PostStatCard({ post, statsData }) {
  ...
  return (
    <div
      style={{
        background: '#ffffff', border: '1.5px solid #e8e4ff',
      ...
      onClick={() => setOpen(v => !v)}
    >
      <style>{`.psc:hover { border-color: #c4b5fd !important; box-shadow: 0 4px 20px rgba(108,99,255,0.12) !important; }`}</style>
      <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'flex-start', gap: 14 }}>
        <div style={{
          width: 4, minHeight: 44, borderRadius: 4, flexShrink: 0, alignSelf: 'stretch',
          background: platforms.length > 0 ? 'linear-gradient(180deg, #6C63FF, #a78bfa)' : '#e8e4f0',
        }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{
            fontSize: 14, color: '#1a1040', margin: '0 0 8px', lineHeight: 1.65, fontWeight: 500,
            overflow: 'hidden', display: '-webkit-box',
            WebkitLineClamp: 2, WebkitBoxOrient: 'vertical',
          }}>
            {post.body || <em style={{ color: '#c4b8e0' }}>Media-only post</em>}
          </p>
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 12, color: '#9c8fc0', fontWeight: 600 }}>
              👁 <strong style={{ color: '#4a3f6b' }}>{totalImpressions.toLocaleString()}</strong> impressions
            </span>
            <span style={{ fontSize: 12, color: '#9c8fc0', fontWeight: 600 }}>
              ♥ <strong style={{ color: '#4a3f6b' }}>{totalLikes.toLocaleString()}</strong> likes
            </span>
            <span style={{ fontSize: 12, color: '#9c8fc0', fontWeight: 600 }}>
        ...
        <span style={{
          fontSize: 13, color: '#b0a5cc', flexShrink: 0,
      ...
      {open && platforms.length > 0 && (
        <div style={{
          borderTop: '1.5px solid #f0edff',
          padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 18,
          background: '#fdfcff', animation: 'fadeIn 0.18s ease',
        }}>
          {platforms.map((pl, i) => {
            ...
            return (
              <div key={i}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 12 }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 800, color: '#1a1040' }}>{info.label}</span>
                  {recordedAt && (
                    <span style={{ fontSize: 11, color: '#b0a5cc', marginLeft: 'auto', fontWeight: 500 }}>
                ...
                {statKeys.length > 0 ? (
                  ...
                  <p style={{ fontSize: 13, color: '#b0a5cc', padding: '8px 0' }}>No stats available yet</p>
                )}
                {pl.records?.length > 1 && (
                  <div style={{ marginTop: 12 }}>
                    <div style={{ fontSize: 10, fontWeight: 800, color: '#c4b8e0', letterSpacing: '0.1em', marginBottom: 8 }}>IMPRESSIONS OVER TIME</div>
      ...
      {open && platforms.length === 0 && (
        <div style={{ borderTop: '1.5px solid #f0edff', padding: '20px', textAlign: 'center' }}>
          <p style={{ fontSize: 13, color: '#b0a5cc' }}>No stats data available for this post</p>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div style={{ animation: 'fadeIn 0.25s ease', background: '#f8f7ff', minHeight: '100%', padding: 2 }}>
      <style>{`
        ...
        .refresh-btn-s:hover { border-color: #6C63FF !important; color: #6C63FF !important; background: #f0edff !important; }
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
            ...
            <h1 style={{ fontSize: 26, fontWeight: 900, color: '#1a1040', letterSpacing: '-0.03em', margin: 0 }}>Analytics</h1>
          </div>
          <p style={{ fontSize: 13, color: '#7c6fa0', margin: 0, fontWeight: 500 }}>
            Performance for <span style={{ color: '#6C63FF', fontWeight: 700 }}>{selectedGroup.name}</span>
          </p>
        </div>
        <button
          className="refresh-btn-s"
          onClick={() => fetchPostsAndStats(selectedGroup.id)}
          style={{ display: 'flex', alignItems: 'center', gap: 7, background: '#ffffff', border: '1.5px solid #e2dcff', borderRadius: 10, padding: '9px 18px', fontSize: 13, color: '#7c6fa0', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14, marginBottom: 24 }}>
        {[
          { label: 'Total Impressions', value: totals.impressions, icon: '👁', color: '#6C63FF', grad: 'linear-gradient(135deg, #6C63FF, #a78bfa)' },
          { label: 'Total Likes',       value: totals.likes,       icon: '♥', color: '#dc2626', grad: 'linear-gradient(135deg, #f87171, #dc2626)' },
          { label: 'Total Comments',    value: totals.comments,    icon: '💬', color: '#2563eb', grad: 'linear-gradient(135deg, #60a5fa, #2563eb)' },
          ...
          <div key={s.label} style={{
            ...
            <div style={{ fontSize: 11, color: '#9c8fc0', marginTop: 7, fontWeight: 700, letterSpacing: '0.04em' }}>{s.label.toUpperCase()}</div>
          </div>
        ))}
      </div>
      ...
      {platformsInStats.length > 0 && (
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
          {['all', ...platformsInStats].map(pid => {
            const info = pid === 'all' ? { label: 'All Platforms', color: '#6C63FF' } : getPlatformInfo(pid)
            ...
            return (
              <button key={pid} className="filter-pill" onClick={() => setPlatformFilter(pid)} style={{
                ...
                background: isActive ? (pid === 'all' ? '#ede9fe' : info.color + '15') : '#ffffff',
                border: `1.5px solid ${isActive ? (pid === 'all' ? '#c4b5fd' : info.color + '55') : '#e2dcff'}`,
                color: isActive ? (pid === 'all' ? '#6d28d9' : info.color) : '#7c6fa0',
      ...
      {loadingPosts ? (
        ...
        <div style={{
          ...
          background: '#ffffff', border: '1.5px solid #e8e4ff', borderRadius: 20,
          boxShadow: '0 2px 16px rgba(108,99,255,0.07)',
        }}>
          <div style={{ fontSize: 44, marginBottom: 16, opacity: 0.25 }}>◎</div>
          <p style={{ fontSize: 16, fontWeight: 700, color: '#7c6fa0', marginBottom: 6 }}>No published posts yet</p>
          <p style={{ fontSize: 13, color: '#b0a5cc' }}>Stats appear once posts are published to platforms</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: '#b0a5cc', letterSpacing: '0.1em', marginBottom: 4 }}>
